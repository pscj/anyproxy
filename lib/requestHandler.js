  ...
  return function (req, cltSocket, head) {
    ...
    */
    co(function *() {
      ...
      var result = yield userRule.beforeDealHttpsRequest(requestDetail);

      if(result.statusCode == 407) {
        return new Promise((resolve) => {
          cltSocket.write("HTTP/1.1 407 Proxy Authentication Required\r\n");
          cltSocket.write("Proxy-Authenticate: Basic realm=\"switch auth\"\r\n\r\n");
          cltSocket.end();
        })
      }
      ...
      .then((serverInfo) => {
        ...
        return new Promise((resolve, reject) => {
          ...
          conn.on('data', (data) => {
            co(function *() {
              try {
                let stopConn = yield userRule.beforeSendHttpsResponse(requestDetail, data.length);
                if(stopConn) {
                  logUtil.debug('stop conn');
                  conn.destroy();                    
                }
              } catch (e) { }
            })          
